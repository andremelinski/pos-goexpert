...
func (c *Category) FindById(coursrId string) (Category, error){
	category := Category{}
	err := c.db.QueryRow("SELECT c.id, c.name, c.description FROM categories c c.id = $1", coursrId).Scan(&category.ID, &category.Name, &category.Description);
